...
##
## Copyright © Francesco Potortì, 2015-2025
...
## done, data are read and a report is created.  If doplot==true
...
## which creates a score.mp4 file instead.
...
## Necessary files are `groundTruth.txt`, 'distance', 'buttonsPressed.log' and
## 'positions.log'.

## The file 'distance' contains one line of text: either "localdistance" or
## "wgs84distance", depending on the reference system used for the
## coordinates. It must be either a local system with unit in meters or else
## WGS84. In the latter case, the coordinates must be in the order lon,lat.

## The files 'buttonsPressed.log' and 'positions.log' are created by the
## stepLogger app.  If they do not exist in a given directory, the menu does not
## offer to use that directory.  However, if dirname is set and dopath==true,
## evaalscore takes 'groundTruth.txt' as a starting point to create two dummy
## files in their place, which emulate an error-free run.

## One can use the create_dummy_positions function to create a synthetic
## 'positions.log' file with random errors for testing.

## If the files relative to a defined scenario
## are present, a graphic report is created as `score.pdf`. Maps should be PNG 8 bit
## image files used as the graph backgrounds, '*.corners' should be text files
## containing the ref system coordinates of the four corners of the map in the
## usual image order: NW, NE, SE, SW.  You can create corners files from
## the `map.png.points` file generated by Qgis, using the function create_corners.
...
## do not use plot for maps and arrows: draw everything by hand
## remove dopath: create a path if no path.pdf or older than data
...
## movie: writeVideo from pkg video does not work with svg and thus loses transparency
...
s.paperposition = [0 0 29.7 21];	# centimetres
...
################ IPIN 2024 scenario (shows only floor 3)
s = struct();
s.reporttitle = "IPIN competition 2024, Hong Kong (CN)";
s.floorpenalty = 15;	# Floor penalty [m] used when computing distance
...
s.mapinfo = struct("map", {"HK-F3",	# file basename
			  },
		   "rm", {0},		# no scaling
		   "title", {"Third floor"},
		   ...
		   "spi", {[1 2 4 5]},	# subplot index
		   "spp", {[-.05 .10 .75 .75]},
		   "psz", [], "p2r", [], "ah", [] # do not touch, internal use
		  );

scenarios.evaal2024 = s; clear s;
...
## Maps info
s.paperorientation = 'landscape';
s.paperposition = [0.3 0 29.4 21];	# centimetres
s.spsr = 3;				# subplot rows
s.spsc = 3;				# subplot columns
s.spi_legend = 1;			# subplot index for legend
s.spp_legend = [0.1 0.65 0.20 0.20];	# explicit legend position
s.spi_distrib = 3;			# subplot index for distribution graph
s.spp_distrib = [0.77 0.65 0.20 0.20];	# distribution graph position
...
function img = fileimread(file)
  img = imread(file);
  ## 2024-10 this one is still there
  if (size(img,3) == 3 && islogical(img))	# work around Octave bug #49140
    img *= uint8(255);				# convert to uint8
#  elseif (isgray(img))				# work around Octave bug #49137
#    img = repmat(img, 1, 1, 3);		# convert to RGB
  endif
endfunction

## Use of text menu when not using gui
function guirunning = __event_manager_enabled__ ()
  ## 2024-10 menu() tries to create a graphics menu even if called with --no-gui
  ## Octave bug #66315
  guirunning = isguirunning;
  return
endfunction

## Work around libgl2ps not handling PNG when generating SVG
if ! strcmp(graphics_toolkit(), 'gnuplot')
  ## 2024-10 still cannot use the Qt graphics toolkit because of Debian bug #1032417
  warning("switching graphics_toolkit from %s to gnuplot\n", graphics_toolkit());
  warning('off', "Octave:gnuplot-graphics");
  graphics_toolkit gnuplot
endif

## Like textscan, but accepts a file name like textread and does not accept options
## Automatically skips up to SKIPHEAD headers and return error on bad format
function out = scan_skip_headers (file, format, skiphead=3)
  for h=0:skiphead			# ignore up to 3 header lines
    fid = fopen(file, 'r');
    try
      unwind_protect			# be sure to close the file after reading error
	## Returns an error if the lines are not conformant to format
	out = textscan(fid, format, 'headerlines', h, 'returnonerror', 0);
      unwind_protect_cleanup
	fclose(fid);			# executed whether error raised or not
      end_unwind_protect
    catch
      continue				# error: try with discarding one more header
    end_try_catch
    return				# textsan returned no error
  endfor
  ## textscan returned and error even after trying skiphead+1 times
  error(lasterror.identifier, lasterror.message);
endfunction

## Draw the path only, without errors
function draw_path (scnr, directory)
  gtruthfile = fullfile(directory, "groundTruth.txt");
  ...
  pathfile = fullfile(directory, "path.pdf");
...
function  create_dummy_buttonpresses (directory, buttonsfile)
  ...
  out = scan_skip_headers(gtruthfile, "%s%f%f%f");
  [l x y f] = out{:};
...
function  create_dummy_positions (directory, n)
  ...
  t = textread(buttonsfile, "%f%*%*%*%*", 'delimiter', ":");
  out = scan_skip_headers(gtruthfile, "%s%f%f%f");
  [l x y f] = out{:};
...
## Create an IMAGE.corners file from IMAGE.png and IMAGE.png.points
function mapcorners = create_corners(image, directory = ".", projection = 'affine', opt=0)
  pkg load image

  imagefile = [ directory "/" image ".png"];
  pointsfile = [ directory "/" image ".png.points"];
  cornersfile = [ directory "/" image ".corners"];
  ...
  out = scan_skip_headers(pointsfile, "%f,%f,%f,%f,%*");
  [mapX mapY pixelX pixelY] = out{:};
...
## Look in BASEDIR for directories containing the required data files and
...
function dir = chosendir(basedir);
  ...
  ## The files expected in a directory with data files.
  ## Optional map files are required to add a background map
  data_files = { "groundTruth.txt",
		 "buttonsPressed.log",
		 "positions.log",
		 "distance" };
  ...
  for i = 1:length(files)
    ## Look inside directories under basedir
    if (isfolder([basedir "/" files{i}])
	&& !(strcmp(".", files{i}) || strcmp("..", files{i})))
      ## Look at files in directories
      dirfiles = readdir([basedir "/" files{i}]);
      data_files_found = 0;
      for j = 1:length(dirfiles)
	data_files_found += any(strcmp(dirfiles{j}, data_files));
      endfor
      if (data_files_found == numel(data_files))
	data_dir_idx(end+1) = i;
      endif
    endif
  endfor
  dirs = files(data_dir_idx);

  ## Cannot find a directory with the required data files
  if (length(dirs) == 0)
    printf("No directory under '%s' contains all the required data files:\n", basedir);
    printf("\t%s\n", data_files{:});
    error("Exiting\n");
  endif

  choice = 0;
  ## See the workaround about graphics menu in function guirunning above
...
## Points P1 and P2 are [lon lat floor] or nx3 matrices
...
function [rl rt rx ry rf et ex ey ef] = process_scenario (s, directory, doplot, domovie)
  ...
  assert(all([bpx; bpy; bpf] == 0),
	 ...
	 gtruthfile, buttonsfile);
  out = scan_skip_headers(gtruthfile, "%s%f%f%f");
  [gtl gtx gty gtf] = out{:};

  ## Check labels and floor
  gtl = strtrim(gtl); bpl = strtrim(bpl); # clean up labels
  assert(numel(gtl) == numel(unique(gtl)), "non-unique labels in %s", gtruthfile);
  assert(numel(bpl) == numel(unique(bpl)), "non-unique labels in %s", buttonsfile);
  assert(all(round(gtf) == gtf), "non-integer floor in %s", gtruthfile);
  ...
  assert(all(found), "%s does not contain all labels in %s\n",
	 gtruthfile, buttonsfile);
  assert(issorted(idx), "labels in %s not sorted as %s", buttonsfile, gtruthfile);
  ...
  ## Check for swapped lat<->lon (should be longitude,latitude)
  mgt = mean([gtx gty]);
  ...
  for m = 1:mapno			# for all the maps
    ...
    mapfile = fullfile(directory, [s.mapinfo(m).map ".png"]);
    cornersfile = fullfile(directory, [s.mapinfo(m).map ".corners"]);
    if (! exist(cornersfile, 'file'))
      warning("Cannot do plot because %s is missing, try with create_corners\n",
	      cornersfile);
      return
    endif
    ...
    axis('xy', 'image', 'manual', 'off', 'nolabel');
    hold on
  endfor
  ...
  for m = 1:mapno
    ...
    ## Ground truth
    ## The series label is unused because no legend is built; it is here for reference only
    plot(s.mapinfo(m).ah,
    	 pbrx(fi,m), pbry(fi,m), "og;ground truth;", 'markersize', msize, 'markerfacecolor', "green");
    axis("equal");
  endfor
  ...
  if (domovie)
    ...
    if (domovie == 2)
      moviefn = fullfile(directory, "score.avi");
	...
	tmpimgfn = "tmpimage.png";	# should use svg instead, to preserve transparency
	saveas(1, tmpimgfn);		# should use getframe, which does not work with gnuplot
    ...
    set(gcf, 'papertype', 'a4', 'paperorientation', s.paperorientation,
	'paperunits', 'centimeters');
    isempty(s.paperposition) || set(gcf, 'paperposition', s.paperposition);
    ...
    for m = 1:mapno
      ...
      ## The series labels are unused because no legend is built; they are here for reference only
      plot(s.mapinfo(m).ah,
	   ...
	   pbex(fi&wfidx,m), pbey(fi&wfidx,m), "or;wrong floor;", 'markerfacecolor', 'red', 'markersize', msize);
...
## Set default values
exist("basedir") || (basedir = ".");	# look here for data directories
exist("doplot") || (doplot = true);	# make plot in addition to computing numbers
exist("dopath") || (dopath = false);	# generate path.pdf
exist("domovie") || (domovie = false);	# make a movie
exist("showplot") || (showplot = false); # plot debugging; reset in production

if domovie
  ## 2023-09 ffmpeg shipped with Octave on Windows cannot read svg
  if (!ispc() && exist("mkmovie", 'file') == 2)
    domovie = 2;
  else
    domovie = 1;
...
if (notfound)
  ...
  if (doplot)
    printf("Asking for plots, but no known scenario in directory \"%s\": setting doplot=false\n",
	  dirname);
    doplot = false;
  endif
endif
...
if (doplot && isempty(pkg('list', 'image')))
...
## Check for buttonsfile and positionsfile
...
[rl rt rx ry rf et ex ey ef] = process_scenario(scnr, directory, doplot, domovie);
...
if (doplot && !domovie)
  ...
  subplot(scnr.spsr, scnr.spsc, scnr.spi_legend);

  ## Use a ready-made legend image from file `legend.png` (deprecated)
  ## or else create it anew (normal behaviour starting with 3.24)
  legendfile = fullfile(directory, "legend.png");
  if (exist(legendfile, 'file'))
    imshow(fileimread(legendfile));	# use image from legendfile as the legend
  else

    ## Create legend
    ## Make a plot with a single point at 0,0 using the same characteristics as the map plots.
    ## The axes are made invisible and the point at 0,1 is made white with an additional series,
    ## so only the legend is shown
    ## TODO: do not rewrite those characteristics here: rather, copy them from the map plots
    ## TODO: rather than using a white series, make the legend background non-trasparent
    set(gca, "defaultlinemarkersize", 2.5);
    plot(0, 0, "-b", 'linewidth', 4, ...
	 0, 0, "-r", ...
	 0, 0, "og", 'markerfacecolor', 'green', ...
	 0, 0, "ob", ...
	 0, 0, "or", 'markerfacecolor', 'red', ...
	 0, 0, "ow", 'markerfacecolor', 'white');
    axis('off');
    lh = legend("estimated path", "errors", "ground truth", "estimated points", "wrong floor");
    isempty(scnr.spp_legend) || set(gca, 'position', scnr.spp_legend);

  endif

  ## Global title
  gtposition = [0.4 0.925 0.2 0.2];
  annotation('textbox', gtposition,
	     'horizontalalignment', 'center', 'fontsize', 15,
	     'interpreter', 'none',
	     'string', dirname);
  ## TODO: rtoffset should be set per scenario
  rtoffset = [-0.3 -0.05 0 0];
  annotation('textbox', gtposition + rtoffset, 'edgecolor', "white",
	     'horizontalalignment', 'center', 'fontsize', 12,
	     'interpreter', 'none',
	     'string', scnr.reporttitle)
  ## Distribution graph
...
fprintf(fid, "\
Error statistics [m]: mean: %.1f  RMS: %.1f  median: %.1f  90th perc.: %.1f\
\n\n",
	mean(poserr), std(poserr, 1), quantile(poserr, [0.5 0.9], 1, 7));
fprintf(fid, "\
